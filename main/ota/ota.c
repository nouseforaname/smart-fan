...
esp_err_t init_ota(esp_ota_firm_t * ota_firm)
{
    ...
    if (ota_firm->updating == NULL) {
        ESP_LOGE(OTA_TAG, "Error: init_ota, unable to find OTA update partition");
        return ESP_FAIL;
    }
    ...
    if (err != ESP_OK) {
        ESP_LOGE(OTA_TAG, "Error: init_ota, esp_ota_begin failed error=%d", err);
        return ESP_FAIL;
    }
...
esp_err_t write_ota(esp_ota_firm_t * ota_firm, char * buf, size_t len)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(OTA_TAG, "Error: write_ota, esp_ota_write failed err=0x%x", err);
        return ESP_FAIL;
    }
...
esp_err_t end_ota(esp_ota_firm_t * ota_firm)
{
    if (esp_ota_end(ota_firm->handle) != ESP_OK) {
        ESP_LOGE(OTA_TAG, "Error: end_ota, esp_ota_end failed!");
        return ESP_FAIL;
    }

    esp_err_t err = esp_ota_set_boot_partition(ota_firm->updating);
    if (err != ESP_OK) {
        ESP_LOGE(OTA_TAG, "Error: end_ota, failed to set boot partition err=0x%x", err);
        return ESP_FAIL;
    }
